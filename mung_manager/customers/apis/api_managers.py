...
class CustomerActiveStatusAPIManager(BaseAPIManager):
    VIEWS_BY_METHOD = {
        "GET": CustomerActiveStatusAPI.as_view,
    }

    @extend_schema(
        tags=["고객"],
        summary="고객의 활성화 여부 조회",
        description="""
        Rogic
            - 고객의 활성화 여부 조회 API 입니다.
        """,
        responses={
            status.HTTP_200_OK: VIEWS_BY_METHOD["GET"]().cls.OutputSerializer,
            status.HTTP_401_UNAUTHORIZED: OpenApiResponse(
